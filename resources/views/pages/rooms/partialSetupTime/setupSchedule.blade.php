                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <label for="field-1" class="col-sm-3 control-label"><strong>Control Number</strong></label>

                            <div class="col-sm-5">
                                <input type="text" class="form-control" id="schedID" name="schedID"
                                    disabled>
                            </div>
                            <input type="button" class="btn btn-primary" id="generate" name="generate" value="Generate Data">
                        </div>

                        <div class="form-group">
                            <label for="field-1" class="col-sm-3 control-label"><strong>Room Name</strong></label>

                            <div class="col-sm-5">
                                <input type="text" class="form-control" id="roomNameModal" name="roomNameModal"
                                    disabled>
                            </div> 
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary" disabled id="buttonUpdate">
                            UPDATE
                        </button>
                    </div>
                </div>
    ...
    $(function () {
        ...
        //save instance for schedule
            $('#setupScheduleForm').on('submit', function (e) {
                e.preventDefault(); 
                var getTime_In = $('#timeIn').val();
                var getTime_Out = $('#timeOut').val(); 
        ...
        // $('#timeInModal').val('10:00:00 AM');
        //update instance
        $('#addRoleForm').on('submit', function (e) {
            e.preventDefault();
            var TimeIN = $('#timeInModal').val();
            var schedID = $('#modalUpdaId').val();
            var TimeOut = $('#timeOutModal').val();
            var getDataModal = [];
            var cboxes = document.getElementsByName('checkDayModal');
            for (var i = 0; i < cboxes.length; i++) {
                getDataModal.push(cboxes[i].checked ? 1 : 0);
            }
            var getRoomName = $('#roomNameModal').val();
            $.ajax({
                type: "POST",
                url: "/setTimeUpdate/calendar/" + $('#schedID').val(),
                data: $(this).serialize() + '&' + 'roomNameModal=' + roomNameModal + '&' +
                    'daysModal=' + getDataModal,
        ...
        $('#generate').on('click',function(){
            $.ajax({
                type: "GET",
                url: "/setTime/getEdit/" + $('#schedID').val(), 
                success: function (response) { 
                    $('#roomNameModal').val(response.roomName);
                    $('#timeInModal').val(response.time_in);
                    $('#timeOutModal').val(response.time_out); 
