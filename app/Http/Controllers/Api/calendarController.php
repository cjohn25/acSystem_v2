...
{ 
        ...
        if ($request->ajax()) { 
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<input type="button" value="&nbsp;&nbsp;Edit&nbsp;&nbsp;" class="edit btn btn-primary btn-sm editProduct" id="edit" data-toggle="modal" data-target="#roleModal" name="edit" onclick="editFunction('.$row->sched_ID.')"/>';
                        $btn = $btn.'<input type="hidden" value="'.$row->sched_ID.'" id="modalUpdaId"> 
                        <a href="/rooms/ShowDeleteSchedule/'.$row->sched_ID.'" data-toggle="tooltip" id="delete"  
    ...
    public function getEdit($id){
        ...
        return timeschedulesetup::where('sched_ID','=',$id) 
        ->join('device','timeschedulesetup.device_ID','=','device.device_ID')
        ->join('rooms','timeschedulesetup.room_ID','=','rooms.id')
        ->first(); 
    } 
    ...
    //update setup time schedule
    public function updateSchedule(Request $request, $id){  
        
        $request->validate([
            'roomNameModal' => 'required',    
            'timeInModal' => 'required',
            'timeOutModal' => 'required'
        ]);  

        // $arrayDays = explode(',', $request->get('daysModal'));  
        $getDataByID = timeschedulesetup::where('sched_ID','=',$id)->first(); 
        $getDataByID->set_day = $request->get('daysModal');
        $getDataByID->time_in = $request['timeInModal'];
        $getDataByID->time_out = $request['timeOutModal']; 
        $getDataByID->save();
